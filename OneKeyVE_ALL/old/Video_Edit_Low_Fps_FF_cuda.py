'''使用python实现以下功能需求：批量降低视频帧率 - CUDA加速FFmpeg实现
功能:
1. 查找当前目录下所有MP4文件
2. 为每个视频创建低帧率版本(3fps)
3. 不保留音频轨道
4. 优先使用NVIDIA GPU加速编码(H.264)，回退到CPU编码
5. 输出文件放在原视频文件所在目录中的output目录内（如果没有则创建）
6. 跳过已存在的文件
7. 显示详细处理信息
8. 增强错误处理和用户反馈
9. 完整的异常处理和鲁棒性设计
...
# 全局变量，用于信号处理
interrupted = False
use_cuda = False  # 全局标志：是否使用CUDA加速
...
def check_cuda_encoder() -> bool:
    ...
    try:
        ...
        if result.returncode != 0:
            logger.warning("无法检查CUDA编码器支持")
            return False

        # 检查输出中是否包含h264_nvenc
        encoder_list = result.stdout.lower()
        if 'h264_nvenc' in encoder_list:
            logger.info("检测到支持CUDA的h264_nvenc编码器")
            return True
        ...
        try:
            ...
            if gpu_check.returncode == 0:
                logger.warning(
                    "检测到NVIDIA GPU，但FFmpeg未编译CUDA支持。请安装支持CUDA的FFmpeg版本。")
            else:
                logger.info("未检测到NVIDIA GPU或驱动程序")
        except FileNotFoundError:
            pass

        return False
    except Exception as e:
        logger.error(f"检查CUDA编码器时出错: {str(e)}")
        return False
...
def get_video_info(video_path: str) -> dict:
    ...
    try:
        ...
        logger.debug(f"获取文件大小时出错: {str(e)}")
    ...
    try:
        ...
        if result.returncode == 0:
            try:
                ...
                logger.debug(f"解析JSON时出错: {str(e)}")
    except Exception as e:
                ...
                if "\n" in line_buffer:
                    ...
                    if any(kw in full_line for kw in ["frame=", "fps=", "time=", "bitrate=", "speed="]):
                        ...
                        if current_time - last_update > 1 or "speed=" in full_line:
                            logger.info(f"[{video_name}] {full_line}")
...
def process_video(video_path: str, use_cuda: bool) -> bool:
    """处理单个视频文件"""
    global interrupted
    if interrupted:
    ...
    try:
        ...
        logger.info(f"使用{'CUDA GPU加速' if use_cuda else 'CPU软件'}编码")
        ...
        if use_cuda:
            cmd = [
                ...
                '-hwaccel_output_format', 'cuda',  # 指定输出格式
                '-i', video_path,
                '-an',                    # 不包含音频
                '-vf', 'fps=3,hwupload',  # 设置帧率并上传到GPU
                '-c:v', 'h264_nvenc',     # 使用NVIDIA GPU H.264编码
                '-preset', 'p7',          # 最高质量预设 (p1-p7, p7最慢质量最高)
                '-tune', 'hq',            # 高质量优化
                '-rc', 'vbr_hq',          # 高质量可变比特率
                '-b:v', '0',              # 忽略比特率限制
                '-cq', '23',              # 恒定质量 (0-51, 值越小质量越高)
                '-profile:v', 'high',     # 高级profile确保兼容性
                '-pix_fmt', 'p010le',     # 10-bit内部处理，输出时自动转换
                '-y',                     # 覆盖输出文件
                output_path
            ]
        else:
        ...
        if success:
            ...
            # 如果使用CUDA失败，尝试回退到CPU
            if use_cuda:
                logger.warning("CUDA编码失败，尝试回退到CPU软件编码...")
                return process_video(video_path, use_cuda=False)

            return False
    except Exception as e:
        logger.error(f"处理视频 {video_path} 时出错: {str(e)}", exc_info=True)
        return False


def main():
    """主函数"""
    global interrupted, use_cuda
    logger.info("=== 视频帧率降低工具 (CUDA加速版) ===")
    logger.info(
    ...
    logger.info(f"编码模式: {'CUDA GPU加速' if use_cuda else 'CPU软件'}")
    logger.info("")
    ...
    for i, video_file in enumerate(video_files, 1):
        ...
        result = process_video(video_file, use_cuda)
        if result:
            success_count += 1
        else:
            failure_count += 1
